...
echo "profile-resolve.sh — shape: files.prTemplateHeadings / files.prTemplateSource"

# These two are read by value TYPE downstream (pr-template-check.sh), not by presence, so a
# malformed one must be rejected here rather than pass through resolution and be read as
# "not set" — jq's `//` treats `false` the same as absent, which is exactly the shape a
# maintainer typo (or a profile generated by something other than a human) could produce.
jq '.defaults.files.prTemplateHeadings = "## Human overview"' "$EXAMPLE" > "$d/headings-scalar.json"
run "$RESOLVE" --profile "$d/headings-scalar.json" --validate
...
jq '.defaults.files.prTemplateHeadings = ["## ok", ""]' "$EXAMPLE" > "$d/headings-emptyelem.json"
run "$RESOLVE" --profile "$d/headings-emptyelem.json" --validate
expect_status "an empty-string element is rejected — non-empty per element" 1

jq '.defaults.files.prTemplateHeadings = []' "$EXAMPLE" > "$d/headings-empty.json"
run "$RESOLVE" --profile "$d/headings-empty.json" --validate
expect_status "an empty array is valid shape (a profile with nothing to check, oddly written)" 0

jq '.defaults.files.prTemplateSource = false' "$EXAMPLE" > "$d/source-false.json"
run "$RESOLVE" --profile "$d/source-false.json" --validate
expect_status "prTemplateSource: false is rejected, not read as absent" 1
expect_match "naming the key" "defaults.files.prTemplateSource must be a non-empty string"

jq '.defaults.files.prTemplateSource = ""' "$EXAMPLE" > "$d/source-empty.json"
run "$RESOLVE" --profile "$d/source-empty.json" --validate
expect_status "prTemplateSource: an empty string is rejected too" 1

jq '.defaults.files.prTemplateSource = 42' "$EXAMPLE" > "$d/source-number.json"
run "$RESOLVE" --profile "$d/source-number.json" --validate
expect_status "prTemplateSource: a number is rejected" 1

jq 'del(.defaults.files.prTemplateHeadings, .defaults.files.prTemplateSource)' "$EXAMPLE" > "$d/nokeys.json"
run "$RESOLVE" --profile "$d/nokeys.json" --validate
expect_status "neither key set at all is still valid — both are optional" 0
