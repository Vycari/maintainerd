<!-- GENERATED COPY — DO NOT EDIT.
     Canonical source: plugins/core/references/config-schema.md
...
```jsonc
{
  ...
  "depsFlow": {
    ...
    "mergeMethod":     "squash",               // "squash" | "merge" | "rebase" — must be allowed by the repo's settings, and must name the merge queue's own merge_method (compared case-insensitively) where the target branch has a merge-queue rule
...
- `depsFlow.*` *(the `dependabot` skill — the only maintainerd skill that merges)* — **the one block
  ...
  a method the repo allows — and where the target branch carries a merge-queue rule it must name that
  queue's own `merge_method` (compared case-insensitively: the config is lowercase, the API uppercase),
  since the queue merges with its method and not the one the skill passes; a mismatch merges nothing and is reported rather than silently overridden; `requireApproval` *(`false`)* additionally demands an explicit approving
