...
- If `config.language` is **neither** `python` nor `typescript`, there is no language block to run; fall back to the **language-generic** categories only — oversized files (by plain line count, flagged against `config.audits.oversizedLines`), DRY violations, and weak abstractions — plus Repo-invariant drift, and **say so in the report**.
...
**Threshold tuning.** The oversized-files threshold is per-repo config, not a hardcoded number: `config.audits.oversizedLines` (default 500) sets the line count every language block flags against, and the per-file-type bumps (a models/router file, a main entry file) stay relative to it — see `references/language-detection.md`. Tune it directly in `.claude/maintainerd.json`, no PR to this skill required; a large repo whose oversized-files category is flagging dozens of merely-large files and finding no real outliers is a sign the value is too low for that codebase, not that the category is broken. The remaining size thresholds in the category tables (15 public methods/members, etc.) are first-cut defaults — ratchet them tighter once the obvious offenders have been split. For example, once a chronically-large file drops below its bar, lower the threshold so the next overgrowth gets caught early. Don't loosen thresholds to make a noisy category quiet; that defeats the audit. Edit those in [`references/language-detection.md`](references/language-detection.md) (or a repo-local override) so future runs pick them up — that file is now the single home for anything language-specific.
