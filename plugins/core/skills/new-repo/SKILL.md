...
4. **Branch protection is a PUT that replaces the whole object.** Send the complete desired state —
   ...
   omits. That body is built from the branch as read, with the profile's opinions laid over it, so
   every protection the profile is silent about survives; **don't hand-write one**, and don't apply
   it if the protection read failed. What it cannot preserve is printed as a warning — a check the
   repo requires that the profile doesn't name, and a check the profile adds to a branch whose
   existing checks are app-pinned. Say those out loud before asking, and let the operator fix the
   profile instead if that is the right answer.
